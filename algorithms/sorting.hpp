...
class Heapsort : public Sorting<Comparable> {
 ...
 private:
  void Sink(std::vector<Comparable> &nums, size_t parent, size_t count) {}

  void heapify(std::vector<Comparable> &arr, int n, int i) {
...
template <typename Comparable>
class Radixsort : public Sorting<Comparable> {
 public:
  int Sort(std::vector<Comparable> &arr) { return 0; }
};

template <typename Comparable>
class Shuffle : public Sorting<Comparable> {
 public:
  int Sort(std::vector<Comparable> &arr) override {
    size_t count = arr.size();
    for (size_t i = count; i > 0; i--) {
      size_t j = aux::Random<size_t>(0, count - 1);

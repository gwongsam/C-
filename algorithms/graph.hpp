...
class Graph : public IConvertible {
 public:
  ...
  void ReadGraph(std::string pwd) {
    std::vector<std::string> data = aux::ReadLines(pwd);
    for (size_t i = 2; i < data.size(); i++) {
      std::vector<std::string> line = aux::SplitDelim(data[i], " ");
      if (line.size() < 2) {
        continue;
      }
  ...
  size_t Edges() {
    ...
    if (edges % 2 != 0) throw invalid_argument("wrong edges can not be modulo");
    edges /= 2;
    return edges;
  }
